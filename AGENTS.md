...
- `internal/projects` — актуальная модель: проект-контейнер (сайт/API/игра/бот) + monitors `http` / `heartbeat`. Удаление мягкое (`deleted_at`), требует заголовка `X-Confirm-Delete: true`. Списки без пагинации (первые 100).
- `internal/monitoring` — планировщик через `FOR UPDATE SKIP LOCKED` (`next_check_at` = защита от двойного захвата), безопасный HTTP-checker (DNS/IP-проверка перед каждым соединением и redirect, anti-SSRF), heartbeat (`204` и для неизвестного токена), Telegram только на открытие/закрытие инцидента.
