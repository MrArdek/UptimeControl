...
Защищённый SSE-поток живого Dashboard. Требует действующую cookie-сессию и учитывает `BASE_PATH`.

После подключения сервер сразу отправляет событие `snapshot`, затем обновляет его каждые 5 секунд:

```text
retry: 5000

id: 1789382400000000000
event: snapshot
data: {"projects":{"projects":[],"next_cursor":null},"incidents":[],"generated_at":"2026-09-14T00:00:00Z"}
```

- `projects` содержит до 500 проектов текущего владельца с monitors;
- `incidents` содержит только открытые инциденты monitors текущего владельца;
- `generated_at` позволяет интерфейсу определить устаревший снимок;
- отозванная сессия получает `session-expired`, после чего поток закрывается;
